...
6. **Oracle hard gate.** When a thread defines `<PROJECT_ROOT>/.roundtable/oracles.yaml`, the executor must run the oracle gate after the scope check and before review. Any `must_pass` oracle failure blocks review and re-dispatches the executor with the failing oracle name and stdout tail.
...
- **Project lessons** — `build_prompt` injects optional lessons from `<PROJECT_ROOT>/.roundtable/memory/lessons.(md|jsonl)` via `scripts/lib/lessons_inject.py`; missing memory is a silent skip.
...
- **Cursor hooks (optional)** — repo ships `hooks/*.sh` + `templates/hooks.json.tmpl`. Native install: `scripts/install_hooks.sh`; bundled path embeds the same commands into `.claude/settings.json` after `<SKILL_DIR>` substitution (`roundtable-setup` §4). See `hooks/README.md`.
- **Codex resume + profiles** — `codex_turn.sh` supports `--mode fresh|refine`, `--no-resume`, `--force-resume`; eligible roles persist `.codex_session.*.json`. Optional `backend.sh apply-codex-profile` installs `templates/codex_profiles.toml.tmpl` into `~/.codex/config.toml`.
- **Budget ledger (real_usd)** — when provider usage is present in `trace.jsonl` / `last.json`, `finalize_turn` stores `real_usd` in `.budget_ledger.jsonl`; `check_budget.py` prefers `real_usd` over legacy `est_usd`.
- **Research artifact pool** — turns that WebSearch/WebFetch MUST log findings under `<thread>/artifacts/research/research-<actor>-<ts>.md` + `.jsonl` (role prompts); `build_prompt` injects a short index so fresh actors avoid redundant searches.
