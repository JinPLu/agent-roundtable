...
5. **Minimal tool disablement.** Agent CLIs run with their full tool surface (Read, Write, Bash, WebSearch, WebFetch). Reviewer roles **and planner roles** get vendor-enforced read-only (Claude `--permission-mode plan`, Codex `--sandbox read-only`, Cursor subagent `Task(readonly:true)`); planner output is captured and written under `artifacts/plan-<actor>-*.md`; only destructive git operations are blocked for write roles.
